...
export const ContactFormPage = () => {
  ...
  return (
    <section className="contactPage-form">
      <div 
        className="contactPage-form__bg" 
        
      />
      
      <div className="contactPage-form__container">
        <div className="contactPage-form__card">
          <h2 className="contactPage-form__title">Оставить заявку на консультацию</h2>

          <form onSubmit={handleSubmit} className="contactPage-form__form">
            {/* Выбор услуги */}
            <div className={`contactPage-form__field ${errors.service ? 'error' : ''}`}>
              <div className="contactPage-form__select-wrapper">
                <div 
                  className="contactPage-form__select-header"
                ...
                {isDropdownOpen && (
                  <div className="contactPage-form__select-dropdown">
                    {serviceOptions.map((option) => (
                      <button
                        key={option.value}
                        type="button"
                        className={`contactPage-form__select-option ${
              ...
              {selectedService && (
                <div className="contactPage-form__select-display">
                  <button
                    type="button"
                    className="contactPage-form__select-display-btn"
                    onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  >
              ...
              {errors.service && (
                <span className="contactPage-form__error-message">Выберите услугу</span>
              )}
            </div>

            {/* Имя */}
            <div className={`contactPage-form__field ${errors.name ? 'error' : ''}`}>
              <label htmlFor="name-input" className="contactPage-form__label">
                Имя
              </label>
              <input
                id="name-input"
                ...
                className="contactPage-form__input"
              />
              {errors.name && (
                <span className="contactPage-form__error-message">Введите имя</span>
              )}
            </div>

            {/* Телефон */}
            <div className={`contactPage-form__field ${errors.phone ? 'error' : ''}`}>
              <label htmlFor="phone-input" className="contactPage-form__label">
                Телефон
              </label>
              <div className="contactPage-form__phone">
                <span className="contactPage-form__flag">🇷🇺</span>
                <input
                  id="phone-input"
                  type="tel"
                  value={phone}
              ...
              {errors.phone && (
                <span className="contactPage-form__error-message">Введите телефон</span>
              )}
            </div>
            ...
            {/* Согласие */}
            <label className={`contactPage-form__consent ${errors.consent ? 'error' : ''}`}>
              <input
                type="checkbox"
                checked={isConsent}
                onChange={(e) => {
            ...
            {errors.consent && (
              <span className="contactPage-form__error-message contact-form__error-message--consent">
